                ...
                <li class="menu menu-hover">
                    @if(isset(Auth::user()->id))
                        @if(Auth::user()->super_user == 1)
                            <a href="{{ URL::to('admin/dashboard') }}"><span
                                        class="icon-man-people-streamline-user icon"></span> Return to Admin
                                Dashboard</a>
                            <ul class="submenu">
                                <li><a href="{{ URL::to('logout') }}">Logout</a></li>
                            </ul>
                        @else
                            <a href="{{ URL::to('dashboard') }}"><span
                                        class="icon-man-people-streamline-user icon"></span> Return to Dashboard</a>
                            <ul class="submenu">
                                <li><a href="{{ URL::to('dashboard/edit/' . Auth::user()->id . '')  }}">Edit your
                                        account</a></li>
                                <li><a href="{{ URL::to('logout') }}">Logout</a></li>
                            </ul>
                        @endif
                    @else
                        <a href="{{ URL::to('login') }}"><span class="icon-man-people-streamline-user icon"></span>
                            Login</a>
                        <ul class="submenu">
