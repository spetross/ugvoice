    ...
    <div class="timeline-panel bordered-2 bordered-azure">
        ...
        <div class="timeline-body">
                    ...
                    @if($post->pictures->count() > 1)
                        <div class="images">
                            @foreach($post->pictures as $picture)
                                <a href="{{ $picture->path }}">
                                    <img data-src="{{ $picture->path }}" src="" class="ui small post image">
                                </a>
                            @endforeach
                        </div>
                        <p>{{ $post->content }}</p>
            ...
            <div class="ui text post menu no-margin">
                ...
                @if(Auth::id() !== $post->user->id)
                    <a data-post="{{$post->id}}" class="item">
                        <i class="uk-icon-thumbs-o-up"></i> Like
                    </a>
                @endif
            ...
            <div
                    ...
                    data-limit="{{ $limit }}"
                    data-post="{{ $post->id }}">
                @include('client.posts.comments', compact('comments'))
            </div>
            @if(Auth::check())
                <form class="ui comment reply form margin-top-5" data-client="{{ $client->slug }}"
                    ...
                    <div class="field form-actions margin-bottom-10 hidden">
